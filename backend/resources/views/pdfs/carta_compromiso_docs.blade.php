...
<p style="font-size:9pt; margin-bottom:10px;">
  ...
  ({{ $inscripcion->periodo->nombre }}, antes del {{ $inscripcion->periodo->fecha_limite_baja_parcial?->format('d/m/Y') ?? 'fecha por confirmar' }}).
</p>
...
<table class="datos" style="margin-bottom:12px;">
  <tr>
    ...
    <td style="text-align:center; {{ $inscripcion->alumno?->pendiente_certificado_bachillerato ? 'color:#c0392b;' : 'color:#27ae60;' }}">
      @if($inscripcion->alumno?->pendiente_certificado_bachillerato)
        PENDIENTE
      @else
        ENTREGADO
      @endif
